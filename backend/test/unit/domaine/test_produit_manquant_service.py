...
class TestProduitManquantService:
    ...
    def test_renvoie_une_erreur_quand_le_produit_est_inconnu(self):
        ...
        with pytest.raises(ProduitInconnuException):
            self.produit_manquant_service.signale_manquant(NOM_DE_PRODUIT_INCONNU)

    def test_signale_qu_un_produit_est_non_manquant(self):
        produit_manquant = ProduitManquant(NOM_DE_PRODUIT_CONNU)

        self.produit_manquant_service.signale_non_manquant(NOM_DE_PRODUIT_CONNU)

        self.produit_manquant_repository.retire.assert_called_with(produit_manquant)

    def test_renvoie_une_erreur_quand_le_produit_non_manquant_est_inconnu(self):
        self.produit_manquant_repository.est_un_nom_de_produit_connu.return_value = False

        with pytest.raises(ProduitInconnuException):
            self.produit_manquant_service.signale_non_manquant(NOM_DE_PRODUIT_INCONNU)
